...
agent = Agent(
    ...
    seed=os.getenv("AGENT_SEED", "default-seed-change-me"),
...
@agent.on_rest_post(endpoint="/agent/api/orders", request=CreateOrder, response=Order)
async def handle_create_order(ctx: Context, request):
    ctx.logger.info("Received order request")
    new_order = client.create_order(request, agent.address)
